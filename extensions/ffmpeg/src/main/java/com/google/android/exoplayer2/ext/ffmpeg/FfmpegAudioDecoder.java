...
import com.google.android.exoplayer2.ext.ffmpeg.bridge.FfmpegNative;
...
/* package */ final class FfmpegAudioDecoder extends
  ...
  private final String TAG = FfmpegAudioDecoder.class.getSimpleName();
  ...
  private FfmpegNative mNative = FfmpegLibrary.getInterface();
  ...
  public FfmpegAudioDecoder(
    ...
    Log.i(TAG, "mime: " + format.sampleMimeType + "   codec name: " + codecName);
    long nativeContext = mNative.codecInitialize(codecName, format);
    if (nativeContext == 0) {
  ...
  protected @Nullable FfmpegDecoderException decode(
    ...
    if (reset) {
      nativeContext = mNative.reset(extraData);
      if (nativeContext == 0) {
    ...
    int result = mNative.decode(inputData, inputSize, outputData, outputBufferSize);
    ...
    if (!hasOutputFormat) {
      channelCount = mNative.getChannelCount();
      sampleRate = mNative.getSampleRate();
